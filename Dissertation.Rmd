...
First I implemented the model used in [@nowotnyLossShapingEnhances2025], using optimal hyperparameters. The model uses Leaky Integrate-and-Fire neuron model with exponential synapses. The model has a single hidden layer with 512 neurons. The hidden layer is recurrent, being fully connected into itself. I trained the model for 300 epochs, then testing the model version which achieved the greatest accuracy. I did this for the current state-of-the-art loss, $\mathcal{L}_{exp\_sum}$.
...
![Code to implement the mathematics of the Eventprop scheme for a Gaussian based loss.](images/result_of_exp_simple.png){#fig:result_of_exp_simple fig-align=center }
...
![Code to implement the mathematics of the Eventprop scheme for a Gaussian based loss.](images/result_of_gaus_simple.png){#fig:result_of_gaus_simple fig-align=center }
