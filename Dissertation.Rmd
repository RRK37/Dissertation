...
\begin{equation}
\mathcal{L}_{sum\_gaus} = -\frac{1}{N_{batch}} \sum^{N_{batch}}_{m=1}log \frac{exp(\int^{T}_{0} \left( a - b\frac{t}{T} exp \left( \frac{- (t/T)^2}{2c^2 }  \right) \right) V^{m}_{l(m)}(t) dt )}{\sum^{N_{out}}_{k=1}exp(\int^{T}_{0} \left( a - b\frac{t}{T} exp \left( \frac{- (t/T)^2}{2c^2 }  \right) \right) V^{m}_{k}(t) dt)}
\label{eqn:lsumgaus}
\end{equation}
...
\begin{equation}
\tau_{\text{mem}} \tilde{\lambda}_{V,j}^{m'} = -\tilde{\lambda}_{V,j}^m + \delta_{j,l(m)} - \frac{\exp\left(\int a - b\frac{t}{T} exp \left( \frac{- (t/T)^2}{2c^2 }  \right) V_j^m dt\right)}{\sum_{k=1}^{N_{\text{class}}} \exp\left(\int_0^T a - b\frac{t}{T} exp \left( \frac{- (t/T)^2}{2c^2 }  \right) V_k^m dt\right)} \left( a - b\frac{t}{T} exp \left( \frac{- (t/T)^2}{2c^2 }  \right) \right) 
\end{equation}
